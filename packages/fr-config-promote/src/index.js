...
async function getCommands() {
    ...
    yargs
        ...
        .command({
            ...
            builder: cliOptions([]),
            handler: (argv) =>
                getAccessToken(tenantUpperUrl, upperClientConfig, ).then((token) => lockTenants(argv, token)),
        })
        .command({
            ...
            handler: (argv) =>
                getAccessToken(tenantUpperUrl, upperClientConfig, ).then((token) => unlockTenants(argv, token)),
        })
        .command({
            ...
            builder: cliOptions([]),
            handler: (argv) =>
                getAccessToken(tenantUpperUrl, upperClientConfig, ).then((token) => checkPromotionStatus(argv, token)),
        })
        .command({
            ...
            builder: cliOptions([]),
            handler: (argv) =>
                getAccessToken(tenantUpperUrl, upperClientConfig, ).then((token) => runDryRunPromotion(argv, token)),
        })
        .command({
            ...
            builder: cliOptions([]),
            handler: (argv) =>
                getAccessToken(tenantUpperUrl, upperClientConfig, ).then((token) => runPromotion(argv, token)),
        })
        .command({
