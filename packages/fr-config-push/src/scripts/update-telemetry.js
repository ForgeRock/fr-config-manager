...
async function pushConfig(
  ...
  for (const category of categories) {
    ...
    for (const providerFile of providerFiles) {
      ...
      let providerJson = fs.readFileSync(filePath, "utf8");
      if (requestedName && requestedName !== JSON.parse(providerJson).id) {
        continue;
      }
      providerJson = replaceEnvSpecificValues(providerJson, false);
