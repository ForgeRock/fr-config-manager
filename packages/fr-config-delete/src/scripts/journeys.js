...
async function getNodeTypes(tenantUrl, realm, token) {
  const nodesEndpoint = constructAmEndpoint(tenantUrl, realm, `${AUTH_TREE_PATH}/nodes`);
...
async function getNode(tenantUrl, realm, token, nodeId, nodeType, nodeVersion) {
 ...
 */
async function retrieveInnerTreeData(
  tenantUrl,
  token,
  realm,
  journeyName,
...
async function processJourney(
  ...
  for (const [nodeId, nodeInfo] of Object.entries(journey.nodes)) {
    if (!!deleteInnerJourneysFlag && nodeInfo.nodeType === INNER_NODE) {
      ...
      if (node.tree) {
        ...
        console.log(`Inner tree not found in node ${nodeId} of type ${nodeInfo.nodeType}`);
      }
    }
  }
  ...
  for (const [nodeId, nodeInfo] of Object.entries(journey.nodes)) {
    if (dryRun) {
      console.log(
        `Dry run: Deleting journey ${journey._id} node ${nodeId} of type ${nodeInfo.nodeType}`
      );
 ...
 */
async function deleteNode(tenantUrl, realm, token, nodeId, nodeType, nodeVersion) {
...
async function deleteJourneys(tenantUrl, realms, name, token, deleteInnerJourneysFlag, dryRun) {
  ...
  if (name) {
    if (realms.length > 1) {
      console.error("Error: Cannot delete journey by name when multiple realms are provided");
      process.exit(1);
    }
  }
  ...
  try {
    for (const realm of realms) {
      const amEndpoint = constructAmEndpoint(tenantUrl, realm, `${AUTH_TREE_PATH}/trees`);
